...
exports.dashboard = function(req,res){
	...
	if (pg == 'login') {
		...
		//wallet daemon info
		arg.wallet = {
			balance: (parseFloat(data.wallet.balance) ? data.wallet.balance+' '+config.symbol : data.wallet.balance)
	...
	if (payout instanceof Array && !(payout)) {
		...
		for(i=0;i<payout.length;i++) {
			...
			if (p instanceof Array && p.length == 2) {
				...
				result.push({payout:p[0],odds:decimal(odds) ? odds.toFixed(2) : odds});
			}
		}
		return result;
	} else if ((payout)) {
